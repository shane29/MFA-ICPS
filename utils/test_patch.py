...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
        with open('./data/LA/4-test.list', 'r') as f:
...
def test_all_case(model_name, num_outputs, model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=1, nms=0):
    ...
    for image_path in loader:
        ...
        h5f = h5py.File(image_path, 'r')
        image = h5f['image'][:]
        # noisy_image = image + (GAussian_noise(torch.tensor(image), 0.1, 0.2)).numpy()
        label = h5f['label'][:]
        ...
        if save_result:
            ...
            # nib.save(nib.Nifti1Image(att1.astype(np.float32), np.eye(4)), test_save_path + "%02d_att.nii.gz" % ith)
            ...
            if num_outputs > 1:
                ...
                # nib.save(nib.Nifti1Image(att2.astype(np.float32), np.eye(4)), test_save_path + "%02d_att_average.nii.gz" % ith)
            ...
            # nib.save(nib.Nifti1Image(noisy_image[:].astype(np.float32), np.eye(4)), test_save_path +  "%02d_img.nii.gz" % ith)
...
def test_single_case_first_output(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    att_map = np.zeros((num_classes, ) + image.shape).astype(np.float32)
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                # att_map[:, xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] \
                #   = att_map[:, xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] + att

    score_map = score_map/np.expand_dims(cnt, axis=0)
    label_map = (score_map[0]>0.5).astype(np.int)
    att_map = att_map/np.expand_dims(cnt, axis=0)
    if add_pad:
        label_map = label_map[wl_pad:wl_pad+w,hl_pad:hl_pad+h,dl_pad:dl_pad+d]
        score_map = score_map[:,wl_pad:wl_pad+w,hl_pad:hl_pad+h,dl_pad:dl_pad+d]
        att_map = att_map[:,wl_pad:wl_pad+w,hl_pad:hl_pad+h,dl_pad:dl_pad+d]
    # _, att_map = model(test_patch)
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    y_logit, att_logit = net(test_patch)
                    num_outputs = len(y_logit)
                    att_outputs = len(att_logit)
                    y=torch.zeros(y_logit[0].shape).cuda()
                    att=torch.zeros(att_logit[0].shape).cuda()
                    for idx in range(num_outputs):
                        y += y_logit[idx]
                    y/=num_outputs
                    for idx in range(att_outputs):
                        att += att_logit[idx]
                    y/=att_outputs
